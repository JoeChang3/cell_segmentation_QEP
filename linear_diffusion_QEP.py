...
def choose_rank_via_criterion(output_mat):
    """
    稳健版：自动净化输入，并在 np.linalg.svd 不收敛时回退到 randomized_svd。
    目标与原函数一致：返回估计秩 est_d（1..ceil(k*2/3)）。
    """
    import numpy as np
    from math import ceil
    # 可选：若有 scipy & sklearn 可用，启用更稳的实现
    _has_scipy = False
    _has_rand  = False
    try:
    ...
    # 4) 计算准则并选秩（与原逻辑一致）
    # 如果是 randomized_svd，U/s/Vt 只含前 r 个分量，下面按可得分量计算
    k, n = A.shape
    # 有些情况下 randomized_svd 返回的分量数 < max_d，这里做限定
    r_avail = s.shape[0]
...
def fit_qep_1d(y_1d,
               ...
               length_scale=0.2,  # 1D 输入已归一化到[0,1]，0.1~0.5合适
               nu=1.5,
               jitter=1e-3,
               device="cpu"):
    """
...
def fit_qep_separable_2d(obs,
                         ...
                         device="cpu"):
    """
    先按行做 1D QEP，再按列做 1D QEP，返回 (H,W)
    这样避免构造 4e4×4e4 的核矩阵，数值非常稳。
    """
    H, W = obs.shape
    # 1) 行方向平滑
...
# def fit_qep_on_grid_linear(
#     obs,
#     kernel_type="rbf",
#     q_power=2.0,
#     train_iters=200, lr=0.05,
#     length_scale=0.2,          # ← 坐标归一化到[0,1]后，用0.1~0.5作为合理初值
#     nu=1.5,
#     device="cpu",
#     inducing_per_dim=(48, 48), # ← 先别太大；48~64 通常稳
#     jitter=1e-2,               # ← 抖动加大
#     standardize=True,
# ):
#     torch.set_default_dtype(torch.float64)
...
#
#     # 归一化坐标到[0,1]
#     xs = np.linspace(0.0, 1.0, H, dtype=np.float64)
#     ys = np.linspace(0.0, 1.0, W, dtype=np.float64)
#     X1, X2 = np.meshgrid(xs, ys, indexing="ij")
#     X = np.column_stack([X1.ravel(), X2.ravel()])
#     y = obs.astype(np.float64).ravel()
#
#     # 目标标准化（避免尺度过大/过小）
#     if standardize:
#         y_mean = y.mean()
...
#
#             # 长度尺度下界（≥1e-3），再设置初值
#             base_k.register_constraint("raw_lengthscale", GreaterThan(1e-3))
#             base_k.lengthscale = torch.tensor([float(length_scale), float(length_scale)],
#                                               dtype=tx.dtype, device=tx.device)
#
#             # SKI 核
#             m1, m2 = inducing_per_dim
#             ski = GridInterpolationKernel(base_k, grid_size=torch.Size([m1, m2]), num_dims=2)
#             self.covar_module = ScaleKernel(ski)   # 不加 NoiseKernel（版本兼容）
#
#         def forward(self, x):
#             m = self.mean_module(x)
#             K = self.covar_module(x).add_jitter(self._jitter)  # 数值抖动
#             return qpytorch.distributions.MultivariateQExponential(m, K, power=self.power)
#
#     like  = qpytorch.likelihoods.QExponentialLikelihood(
#         power=torch.tensor(POWER, dtype=train_x.dtype, device=train_x.device)
#     )
#     model = ExactQEPModel(train_x, train_y, like).to(device)
#
#     model.train(); like.train()
#     opt = torch.optim.Adam(model.parameters(), lr=lr)
#     mll = qpytorch.mlls.ExactMarginalLogLikelihood(like, model)
...
#             loss = -mll(out, train_y)
#
#             # 若出现 NaN/Inf：增大抖动并跳过这步
#             if not torch.isfinite(loss):
#                 model._jitter = min(model._jitter * 10.0, 1e-1)  # 抖动升级到最多1e-1
...
#             # 训练后参数钳制（避免爆到极端值）
#             with torch.no_grad():
#                 # lengthscale 限在[1e-3, 2.0]（归一化坐标下）
#                 ls = model.covar_module.base_kernel.base_kernel.lengthscale
#                 ls.data.clamp_(1e-3, 2.0)
#                 # outputscale 限在[1e-4, 100]
#                 os = model.covar_module.raw_outputscale
#                 os.data.clamp_(np.log(1e-4), np.log(100.0))
#
#     model.eval(); like.eval()
#     with torch.no_grad(), gpytorch.settings.fast_pred_var():
#         pred = like(model(train_x)).mean.detach().cpu().numpy().reshape(H, W)
#
#     # 反标准化
#     pred = pred * y_std + y_mean
#     return pred

# === 生成“真实场”：1D 线性扩散（隐式Euler），等价 R 里的 ReacTran + deSolve 设定 ===
# def generate_linear_diffusion(k=200, n=200, L=1.0, T=0.2, D=1.0, C_left=0.0, C_right=1.0):
#     """
#     1D 热传导 u_t = D*u_xx - Q，这里 Q=0
#     边界：左 Dirichlet = C_left(=0)，右 Dirichlet = C_right(=1)  —— 对应 R: C.down=Cext
#     隐式 Euler： (I - dt*D*L) u^{t+1} = u^t
#     返回矩阵 reality: shape (k, n)，列是时间步（与 R 一致）
#     """
#     dx = L / k
#     dt = T / (n - 1)
#     # 二阶中心差分的离散拉普拉斯（Dirichlet）
#     main = np.full(k, 2.0)
#     off  = np.full(k - 1, -1.0)
#     Lap = sparse.diags([off, main, off], [-1, 0, 1], shape=(k, k), format="csr") / (dx * dx)
#
#     A = sparse.eye(k, format="csr") - dt * D * Lap  # 隐式Euler矩阵
#
#     # Dirichlet 边界：把边界方程直接设为 u_left=C_left, u_right=C_right
#     # 方式：把 A 第一行/最后一行改为单位行，并在 RHS 里塞边界值
#     A = A.tolil()
#     A[0, :] = 0.0;  A[0, 0] = 1.0
#     A[-1, :] = 0.0; A[-1, -1] = 1.0
#     A = A.tocsr()
#
#     u = np.zeros(k)               # 初值 0（=R 里 y=rep(0, k)）
#     u[0]  = C_left
#     u[-1] = C_right
#
...
def generate_linear_diffusion(k=200, n=200, L=1.0, T=0.2, D=1.0,
                              ...
                              clip_bounds=(0.0, 1.0)):
    """
    稳定版 1D 扩散: Crank–Nicolson (更稳于纯隐式/显式)，Dirichlet 边界。
    返回 reality: shape (k, n)，每列一个时间步。
    """
    # 网格与时间步
    dx = L / k
    dt = T / (n - 1)
...
rmse_qep_rbf     = _alloc(num_repetition, len(sigma0_list))   # R 的 lattice_exp
rmse_qep_matern  = _alloc(num_repetition, len(sigma0_list))   # R 的 lattice_matern
...
for j, sigma0 in enumerate(sigma0_list):
    for it in range(num_repetition):
        ...
        # QEP-Matern（对应 R: lattice_matern）
        ...
        # R 的 DMD 是 cbind(y[,1], in_sample_pred)
...
# ======= Figure 6(B)：三联图（观测均值、含噪观测、Matern 预测）=======
# 取 sigma0_list 的第三档（=0.3），与 R 一致
