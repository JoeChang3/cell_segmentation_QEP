...
source("/Users/zchan/Desktop/projects/Cell_Seg_GP/code/cell_segmentation-main/Nuclear_Real_Analysis/GP_vs_NoGP/Segmentation_Functions_NoGP (no conflict function names).R") #No GP Segmentation
...
save_boundary_figures <- function(folder_path, ori_img_matrix, GP_masks, noGP_Masks, True_Mask) {
  ...
  png(file.path(folder_path, "noGP_boundaries.png"), width = 800, height = 800)
  par(mar = c(0, 0, 0, 0))
  image2D(ori_img_matrix, colkey = FALSE, axes = FALSE)
...
save_ious <- function(folder_path, True_Mask, GP_masks, noGP_Masks) {
  ...
  print("Non-GO IoUs calculated")
  
  # Save IoU Data
  write.csv(ious_gp, file = file.path(folder_path, "ious_gp.csv"), row.names = TRUE)
  write.csv(ious_noGP, file = file.path(folder_path, "ious_noGP.csv"), row.names = TRUE)
...
process_image <- function(folder_name, base_dir) {
  ...
  noGP_Masks_path <- file.path(folder_path, "original_noGP_masks.tif")
...
if (!skip){
  iou_data <- lapply(folders, function(folder_name) {
    ...
    ious_noGP <- read.csv(file.path(folder_path, "ious_noGP.csv"), row.names = 1)
...
plot_data <- combined_table %>%
  ...
  mutate(Method = recode(Method, "GP_Method_AP" = "GP", "noGP_Method_AP" = "No GP"),
         Pair = recode(Pair,
...
plot_box <- ggplot(plot_data, aes(x = as.factor(Threshold), y = AP, fill = Method)) +
  ...
  labs(x = "Threshold", 
       ...
       color = "Method") +
  theme_classic() +
  scale_fill_manual(values = c("GP" = "blue", "No GP" = "red")) +
  scale_color_manual(values = c("GP" = "blue", "No GP" = "red")) +
  theme(
...
print(plot_box)
ggsave("mean_ap_boxplot_comparison (GP vs no GP).png", plot = plot_box, width = 10, height = 8, dpi = 300)
...
plot_line <- ggplot(plot_data, aes(
  ...
  scale_linetype_manual(values = c("GP" = "solid", "No GP" = "dashed")) +
  guides(
...
print(plot_line)
ggsave("ap_comparison_plot (GP vs No GP).png", plot = plot_line, width = 10, height = 8, dpi = 300)
