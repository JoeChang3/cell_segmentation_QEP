...
source("/Users/zchan/Desktop/projects/Cell_Seg_GP/code/cell_segmentation-main/src/Modified_Functions_RGasp.R") #GP Segmentation
source("/Users/zchan/Desktop/projects/Cell_Seg_GP/code/cell_segmentation-main/Nuclear_Real_Analysis/GP_vs_NoGP/Segmentation_Functions_NoImageGP (no conflict function names).R") #No Image GP Segmentation
...
process_image_mask <- function(image_path) {
  image_mask <- readImage(image_path)
  image_mask <- image_mask@.Data
  
  # Extract unique values and map them to consecutive integers
  unique_values <- unique(as.vector(image_mask))
  value_to_integer <- setNames(seq(0, length(unique_values) - 1), unique_values)
  
  # Convert image to integer mask
  integer_mask <- matrix(value_to_integer[as.character(image_mask)], nrow = nrow(image_mask), ncol = ncol(image_mask))
...
compute_ap_table <- function(ious_gp, ious_noGP) {
  ...
  noGP_precisions <- numeric(length(thresholds))
  ...
  for (i in seq_along(thresholds)) {
    ...
    # Compute AP for no GP-based method
    ap_result_noGP <- compute_ap_from_ious(ious_noGP, threshold = threshold)
    noGP_precisions[i] <- ap_result_noGP$precision
    
    # Compare methods: TRUE if GP has higher precision than no GP
    is_gp_better[i] <- gp_precisions[i] > noGP_precisions[i]
  }
  
  ap_table <- data.frame(
    Threshold = thresholds,
    GP_Method_AP = gp_precisions,  
    noGP_Method_AP = noGP_precisions,  
...
#### Save Mask Figures ####
save_boundary_figures <- function(folder_path, ori_img_matrix, GP_masks, noGP_Masks, True_Mask) {
  # Find Boundaries
  GP_boundaries <- find_boundaries(GP_masks)
  noGP_boundaries <- find_boundaries(noGP_Masks)
  ...
  #### Save No GP Masks Figure
  png(file.path(folder_path, "noImageGP_boundaries.png"), width = 800, height = 800)
  par(mar = c(0, 0, 0, 0))
  image2D(ori_img_matrix, colkey = FALSE, axes = FALSE)
  noGP_boundary_coords <- which(noGP_boundaries == 1, arr.ind = TRUE)
  points(noGP_boundary_coords[, 1] / nrow(ori_img_matrix), 
         noGP_boundary_coords[, 2] / ncol(ori_img_matrix), 
         col = "black", pch = 19, cex = 0.5)
  dev.off()
...
save_ious <- function(folder_path, True_Mask, GP_masks, noGP_Masks) {
  # Compute IoUs
  ...
  write.csv(ious_noGP, file = file.path(folder_path, "ious_noImageGP.csv"), row.names = TRUE)
...
process_image <- function(folder_name, base_dir) {
  ...
  noGP_Masks_path <- file.path(folder_path, "original_noImageGP_masks.tif")
  ...
  # Process no GP Masks
  noGP_Masks_result <- generate_GP_Masks_test2(file_path, nugget = T)
  noGP_Masks <- noGP_Masks_result$GP_masks
  
  #Process true masks
  True_Mask <- process_image_mask(True_Masks_path)
  
  # Save Boundary Figures
  save_boundary_figures(folder_path, ori_img_matrix, GP_masks, noGP_Masks, True_Mask)
...
skip = F # Change this if CSV files not available
if (!skip){
  iou_data <- lapply(folders, function(folder_name) {
    folder_path <- file.path(base_dir, folder_name)
    
    # Read GP and no GP IoU CSVs from the folder
    ious_gp <- read.csv(file.path(folder_path, "ious_gp.csv"), row.names = 1)
    ious_noGP <- read.csv(file.path(folder_path, "ious_noImageGP.csv"), row.names = 1)
    
    # Compute AP Table
    ap_table <- compute_ap_table(ious_gp, ious_noGP)
...
plot_data <- combined_table %>%
  pivot_longer(cols = c(GP_Method_AP, noGP_Method_AP), 
               names_to = "Method", values_to = "AP") %>%
  mutate(Method = recode(Method, "GP_Method_AP" = "GP", "noGP_Method_AP" = "No Image GP"),
         Pair = recode(Pair,
                       "../../Image_Data/nuclear_test_images/nuclei_figure_1" = "Image 1",
                       "../../Image_Data/nuclear_test_images/nuclei_figure_2" = "Image 2",
                       "../../Image_Data/nuclear_test_images/nuclei_figure_3" = "Image 3",
                       "../../Image_Data/nuclear_test_images/nuclei_figure_4" = "Image 4",
                       "../../Image_Data/nuclear_test_images/nuclei_figure_5" = "Image 5"))
...
plot_box <- ggplot(plot_data, aes(x = as.factor(Threshold), y = AP, fill = Method)) +
  ...
  labs(x = "Threshold", 
       ...
       color = "Method") +
  theme_classic() +
  scale_fill_manual(values = c("GP" = "blue", "No Image GP" = "red")) +
  scale_color_manual(values = c("GP" = "blue", "No Image GP" = "red")) +
  theme(
...
print(plot_box)
ggsave("mean_ap_boxplot_comparison (GP vs no Image GP).png", plot = plot_box, width = 10, height = 8, dpi = 300)
...
plot_line <- ggplot(plot_data, aes(
  ...
  scale_linetype_manual(values = c("GP" = "solid", "No Image GP" = "dashed")) +
  guides(
...
print(plot_line)
ggsave("ap_comparison_plot (GP vs No Image GP).png", plot = plot_line, width = 10, height = 8, dpi = 300)
