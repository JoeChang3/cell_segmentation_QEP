...
#### Define Folder Paths ####
base_dir <- getwd() 
folders <- paste0("Image_Data/whole_cell_test_images/whole_cell_figure_", 1:5)
...
process_image <- function(folder_name, base_dir) {
  ...
  file_path <- file.path(folder_path, "original_fig.jpg")
  ...
  ap_table$Pair <- folder_name  # Add folder name as label
  return(ap_table)
}
