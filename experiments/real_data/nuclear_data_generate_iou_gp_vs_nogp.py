#### Nuclear Real Analysis — GP vs No GP ####
#
# R source: Nuclear_Real_Analysis/GP_vs_NoGP/
#           Nuclear_Data_Generate_IoU (GP vs no GP).R
#
# Runs the GP and NoGP segmentation pipelines on five nuclear test images,
...
from py_core.segmentation_no_gp import generate_no_gp_masks_test
...
# -----------------------------------------------
# compute_ap_table  (GP vs NoGP variant)
# -----------------------------------------------
def compute_ap_table(ious_gp: pd.DataFrame, ious_nogp: pd.DataFrame) -> pd.DataFrame:
    """
    R: compute_ap_table(ious_gp, ious_noGP)
    Evaluates AP at each threshold in AP_THRESHOLDS for both methods.
    Column 'noGP_Method_AP' matches the R output column name.
    """
    gp_prec   = []
    nogp_prec = []
    gp_better = []
    ...
    for th in AP_THRESHOLDS:
        ...
        nogp = compute_ap_from_ious(ious_nogp,  threshold=th)["precision"]
        gp_prec.append(gp)
        nogp_prec.append(nogp)
        gp_better.append(gp > nogp)

    return pd.DataFrame({
        "Threshold":     AP_THRESHOLDS,
        "GP_Method_AP":  gp_prec,
        "noGP_Method_AP": nogp_prec,
        "GP_Better":     gp_better,
    })
...
def save_boundary_figures(
    ...
    ori_img: np.ndarray,
    gp_masks: np.ndarray,
    nogp_masks: np.ndarray,
    true_mask: np.ndarray,
) -> None:
    """
    R: save_boundary_figures(folder_path, ori_img_matrix, GP_masks, noGP_Masks, True_Mask)

    Saves three 800×800 PNGs (matching R's png(width=800, height=800)) into folder_path:
      GP_boundaries.png, noGP_boundaries.png, True_boundaries.png

    Approximation: R uses image2D() + points() to overlay boundary pixel coordinates
    scaled to [0,1].  Python uses imshow + scatter with pixel coordinates directly.
    skimage find_boundaries(mode='outer') is used instead of the custom 4-connectivity
    find_boundaries_4n(); the visual result is equivalent for cell boundary overlays.
    """
    ...
    _save_one(nogp_masks, "noGP_boundaries.png")
...
def save_ious(
    ...
    Saves ious_gp.csv and ious_noGP.csv into folder_path alongside the images.
    Filenames match R so that previously saved CSVs can be reloaded when SKIP=True.
    Returns the AP comparison table.
    """
    ious_gp   = compute_ious(true_mask, gp_masks)
    ious_nogp = compute_ious(true_mask, nogp_masks)

    print("GP IoUs calculated")
    ious_gp.to_csv(folder_path / "ious_gp.csv",   index=True)

    print("Non-GP IoUs calculated")
    ious_nogp.to_csv(folder_path / "ious_noGP.csv", index=True)

    return compute_ap_table(ious_gp, ious_nogp)
...
#       3. Generate NoGP masks (generate_GP_Masks_test2 in R).
#       4. Load ground-truth mask.
#       5. Save boundary overlay figures.
#       6. Compute IoUs, save CSVs, return AP table.
#
#     R image file: original_fig.png  (PNG, not JPG).
#     R NoGP mask:  original_noGP_masks.tif is listed in R but never used — NoGP
#                   masks are always recomputed by generate_GP_Masks_test2.
#     """
...
#
#     # Generate NoGP masks — same pipeline but without GP image smoothing
#     print(f"  Running NoGP segmentation on {folder_path.name} ...")
#     nogp_result = generate_no_gp_masks_test(str(file_path), nugget=True)
#     nogp_masks  = nogp_result.gp_masks
#
...
#     save_boundary_figures(folder_path, ori_img, gp_masks, nogp_masks, true_mask)
#
#     # Compute IoUs, save CSVs, build AP table
#     ap_table = save_ious(folder_path, true_mask, gp_masks, nogp_masks)
...
def process_image(folder_path: Path) -> pd.DataFrame:
    ...
    print(f"  nogp_masks shape:{nogp_masks.shape}")

    # Align predicted masks to true mask
    gp_masks = align_mask_to_reference(gp_masks, true_mask, name="gp_masks")
    nogp_masks = align_mask_to_reference(nogp_masks, true_mask, name="nogp_masks")
    ...
    ap_table = save_ious(folder_path, true_mask, gp_masks, nogp_masks)
    ap_table["Pair"] = folder_path.name
    return ap_table
...
def make_plots(combined_table: pd.DataFrame, out_dir: Path) -> None:
    ...
    Saves:
      mean_ap_boxplot_comparison (GP vs no GP).png
      ap_comparison_plot (GP vs No GP).png
    into out_dir, matching R's ggsave() filenames.
    ...
    plot_data = pd.concat([
        ...
        df[["Threshold", "Pair"]].assign(Method="No GP", AP=df["noGP_Method_AP"].values),
    ], ignore_index=True)

    thresholds = sorted(plot_data["Threshold"].unique())
    methods    = ["GP", "No GP"]
    # R: scale_fill_manual(values = c("GP" = "blue", "No GP" = "red"))
    method_colors = {"GP": "blue", "No GP": "red"}
    ...
    fig.tight_layout()
    fig.savefig(out_dir / "mean_ap_boxplot_comparison (GP vs no GP).png", dpi=300)
    plt.close(fig)
    ...
    fig.tight_layout()
    fig.savefig(out_dir / "ap_comparison_plot (GP vs No GP).png", dpi=300, bbox_inches="tight")
    plt.close(fig)
...
def main() -> None:
    ...
    if not SKIP:
        ...
        # Note: in R skip=F runs this block; here SKIP=True runs it (name convention flipped).
        all_tables = []
        for folder_path in FOLDERS:
            ious_gp_path   = folder_path / "ious_gp.csv"
            ious_nogp_path = folder_path / "ious_noGP.csv"
            if not ious_gp_path.exists() or not ious_nogp_path.exists():
            ...
            ious_nogp = pd.read_csv(ious_nogp_path,  index_col=0)
            ap = compute_ap_table(ious_gp, ious_nogp)
            ap["Pair"] = folder_path.name
            all_tables.append(ap)
        combined = pd.concat(all_tables, ignore_index=True)

    # Save combined AP table — R does not explicitly save this; Python saves to results/tables/
    out_csv = tables_out / "combined_ap_table_gp_vs_nogp.csv"
