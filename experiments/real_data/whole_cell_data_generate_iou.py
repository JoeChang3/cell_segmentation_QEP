#### Whole Cell Real Analysis — GP vs ImageJ ####
#
# R source: Whole_Cell_Real_Analysis/Whole_Cell_Data_Generate_IoU.R
#
# Runs the GP segmentation pipeline on five whole-cell test images, compares
# the resulting masks to ImageJ-produced masks using IoU / Average Precision,
# and saves comparison plots.
#
# Key difference from the nuclear GP-vs-NoGP script:
#   - The comparison method (ImageJ) is loaded from a pre-existing TIF file
#     rather than being re-segmented from scratch.
#   - Source images are JPEG (.jpg) not PNG.
#
# Path conventions
#   data/whole_cell_test_images/whole_cell_figure_{1..5}/  — image inputs + per-image IoU CSVs
...
DATA_DIR     = PROJECT_ROOT / "data" / "whole_cell_test_images"
RESULTS_DIR  = PROJECT_ROOT / "results"

FOLDERS = [DATA_DIR / f"whole_cell_figure_{i}" for i in range(1, 6)]

# R: skip = T  (R's default runs the skip/CSV block)
...
def process_image(folder_path: Path) -> pd.DataFrame:
    ...
    Processes one whole-cell test image folder:
      1. Load original image (JPEG).
    ...
    Note: unlike the nuclear GP-vs-NoGP script, the comparison method (ImageJ)
    is loaded directly from disk rather than re-computed.
    """
    file_path        = folder_path / "original_fig.jpg"      # R: file.path(folder_path, "original_fig.jpg")
...
def make_plots(combined_table: pd.DataFrame, out_dir: Path) -> None:
    ...
    Saves:
      mean_ap_boxplot_comparison.png
      ap_comparison_plot.png
    ...
    """
    # Recode Pair labels — R: recode(Pair, "whole_cell_figure_1" = "Image 1", ...)
    pair_map = {f"whole_cell_figure_{i}": f"Image {i}" for i in range(1, 6)}
    ...
    fig.tight_layout()
    fig.savefig(out_dir / "mean_ap_boxplot_comparison.png", dpi=300)
    plt.close(fig)
    ...
    fig.tight_layout()
    fig.savefig(out_dir / "ap_comparison_plot.png", dpi=300, bbox_inches="tight")
    plt.close(fig)
...
def main() -> None:
    ...
    out_csv = tables_out / "combined_ap_table_gp_vs_imagej.csv"
