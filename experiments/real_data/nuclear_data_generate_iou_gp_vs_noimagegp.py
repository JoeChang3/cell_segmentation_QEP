#### Nuclear Real Analysis — GP vs No Image GP ####
#
# R source: Nuclear_Real_Analysis/GP_vs_NoGP/
#           Nuclear_Data_Generate_IoU (GP vs no Image GP).R
#
# Runs the GP and NoImageGP segmentation pipelines on five nuclear test images,
# computes per-image IoU matrices, evaluates AP at IoU thresholds 0.5–0.80,
# and saves comparison plots.
#
# "No Image GP" = raw image passed directly to Criterion 1 (no GP fitted to
# image tiles), but the criterion curve IS smoothed with RGaSP (approximated
# here as Gaussian smoothing via criterion_1 in Modified_Functions_RGasp.py).
# Contrast with "No GP" (nuclear_data_generate_iou_gp_vs_nogp.py) which also
# skips image GP but uses no smoothing on the criterion curve either.
#
# Path conventions
...
from py_core.segmentation_no_image_gp import generate_no_image_gp_masks_test
...
# -----------------------------------------------
# compute_ap_table  (GP vs NoImageGP variant)
# -----------------------------------------------
def compute_ap_table(ious_gp: pd.DataFrame, ious_noimagegp: pd.DataFrame) -> pd.DataFrame:
    """
    R: compute_ap_table(ious_gp, ious_noGP)   [same function, different baseline]
    Evaluates AP at each threshold in AP_THRESHOLDS for both methods.
    Column 'noImageGP_Method_AP' matches the R output column name.
    """
    gp_prec       = []
    noimagegp_prec = []
    gp_better     = []
    ...
    for th in AP_THRESHOLDS:
        ...
        noimagegp = compute_ap_from_ious(ious_noimagegp, threshold=th)["precision"]
        gp_prec.append(gp)
        noimagegp_prec.append(noimagegp)
        gp_better.append(gp > noimagegp)

    return pd.DataFrame({
        "Threshold":          AP_THRESHOLDS,
        "GP_Method_AP":       gp_prec,
        "noImageGP_Method_AP": noimagegp_prec,
        "GP_Better":          gp_better,
    })
...
def save_boundary_figures(
    ...
    ori_img: np.ndarray,
    gp_masks: np.ndarray,
    noimagegp_masks: np.ndarray,
    true_mask: np.ndarray,
) -> None:
    """
    R: save_boundary_figures(folder_path, ori_img_matrix, GP_masks, noGP_Masks, True_Mask)
       [noGP_Masks here is the NoImageGP mask]

    Saves three PNGs into folder_path (alongside the images):
      GP_boundaries.png, noImageGP_boundaries.png, True_boundaries.png

    Approximation: R uses image2D() + points() with coordinates scaled to [0,1].
    Python uses imshow + scatter at pixel coordinates.
    skimage find_boundaries(mode='outer') is used instead of the custom
    4-connectivity find_boundaries_4n(); visual result is equivalent.
    """
    ...
    _save_one(noimagegp_masks, "noImageGP_boundaries.png")   # R: "noImageGP_boundaries.png"
...
def save_ious(
    ...
    """
    R: save_ious(folder_path, True_Mask, GP_masks, noGP_Masks)
       [noGP_Masks here is the NoImageGP mask]

    Saves ious_gp.csv and ious_noImageGP.csv into folder_path alongside the images.
    Filenames match R (ious_noImageGP.csv) so that previously saved CSVs can be
    reloaded when SKIP=True.
    Returns the AP comparison table.
    """
    ious_gp        = compute_ious(true_mask, gp_masks)
    ious_noimagegp = compute_ious(true_mask, noimagegp_masks)

    print("GP IoUs calculated")
    ious_gp.to_csv(folder_path / "ious_gp.csv", index=True)

    print("Non-Image-GP IoUs calculated")
    ious_noimagegp.to_csv(folder_path / "ious_noImageGP.csv", index=True)  # R filename

    return compute_ap_table(ious_gp, ious_noimagegp)
...
def align_mask_to_reference(mask: np.ndarray, ref: np.ndarray, name: str = "mask") -> np.ndarray:
    ...
    Current fix:
    - keep as-is if shapes already match
    - otherwise try transpose
    """
...
def process_image(folder_path: Path) -> pd.DataFrame:
    ...
    Processes one nuclear test image folder:
      ...
      3. Generate NoImageGP masks (generate_GP_Masks_test2 from NoImageGP source in R).
      4. Load ground-truth mask.
      5. Save boundary overlay figures.
      6. Compute IoUs, save CSVs, return AP table.

    R image file: original_fig.png (PNG).
    R NoImageGP mask file: original_noImageGP_masks.tif is referenced in R but
    never used — NoImageGP masks are always recomputed by generate_GP_Masks_test2.
    """
    file_path       = folder_path / "original_fig.png"
    ...
    # Generate NoImageGP masks — same pipeline structure as NoGP but criterion
    # curve is Gaussian-smoothed (approximating RGaSP from the R source)
    print(f"  Running NoImageGP segmentation on {folder_path.name} ...")
    ni_result      = generate_no_image_gp_masks_test(str(file_path), nugget=True)
    noimagegp_masks = ni_result.gp_masks
    ...
    print(f"  noimagegp_masks shape:{noimagegp_masks.shape}")

    # Align predicted masks to true mask (same pattern as gp_vs_nogp script)
    gp_masks        = align_mask_to_reference(gp_masks,        true_mask, name="gp_masks")
    noimagegp_masks = align_mask_to_reference(noimagegp_masks, true_mask, name="noimagegp_masks")

    # Save boundary overlay figures into the image folder (alongside images, matching R)
    save_boundary_figures(folder_path, ori_img, gp_masks, noimagegp_masks, true_mask)

    # Compute IoUs, save CSVs, return AP table
    ap_table = save_ious(folder_path, true_mask, gp_masks, noimagegp_masks)
...
def make_plots(combined_table: pd.DataFrame, out_dir: Path) -> None:
    ...
    Saves (matching R's ggsave() filenames):
      mean_ap_boxplot_comparison (GP vs no Image GP).png
      ap_comparison_plot (GP vs No Image GP).png
    into out_dir.

    Approximation: R uses ggplot2 theme_classic() with full font/margin control.
    Python uses matplotlib with equivalent sizes.  Jitter uses np.random.normal
    (same intent as ggplot's geom_jitter(width=0.2)).
    """
    ...
    # Pivot to long form — R: pivot_longer(cols=c(GP_Method_AP, noGP_Method_AP), ...)
    # R: mutate(Method = recode(Method, "GP_Method_AP"="GP", "noGP_Method_AP"="No Image GP"))
    plot_data = pd.concat([
        df[["Threshold", "Pair"]].assign(Method="GP",           AP=df["GP_Method_AP"].values),
        df[["Threshold", "Pair"]].assign(Method="No Image GP",  AP=df["noImageGP_Method_AP"].values),
    ], ignore_index=True)

    thresholds = sorted(plot_data["Threshold"].unique())
    methods    = ["GP", "No Image GP"]
    # R: scale_fill_manual(values = c("GP" = "blue", "No Image GP" = "red"))
    method_colors = {"GP": "blue", "No Image GP": "red"}
    ...
    fig.tight_layout()
    fig.savefig(out_dir / "mean_ap_boxplot_comparison (GP vs no Image GP).png", dpi=300)
    plt.close(fig)
    ...
    fig.tight_layout()
    fig.savefig(out_dir / "ap_comparison_plot (GP vs No Image GP).png", dpi=300,
                bbox_inches="tight")
    plt.close(fig)
...
def main() -> None:
    ...
    if not SKIP:
        ...
        # R: if (!skip){ read.csv(ious_noImageGP.csv); combined_table <- bind_rows(...) }
        # Note: in R skip=F runs this block; here SKIP=True runs it (convention flipped).
        all_tables = []
        for folder_path in FOLDERS:
            ious_gp_path       = folder_path / "ious_gp.csv"
            ious_noimagegp_path = folder_path / "ious_noImageGP.csv"
            if not ious_gp_path.exists() or not ious_noimagegp_path.exists():
            ...
            ious_noimagegp = pd.read_csv(ious_noimagegp_path,  index_col=0)
            ap = compute_ap_table(ious_gp, ious_noimagegp)
            ap["Pair"] = folder_path.name
            all_tables.append(ap)
        combined = pd.concat(all_tables, ignore_index=True)

    # Save combined AP table — R does not explicitly save this; Python saves to results/tables/
    out_csv = tables_out / "combined_ap_table_gp_vs_noimagegp.csv"
